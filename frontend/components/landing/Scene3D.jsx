...
import { Float, MeshDistortMaterial, Sphere, Icosahedron, Box, Torus } from '@react-three/drei';
...
function FloatingShape({ shape = 'sphere', position, color, scale = 1, mouse, scroll, zRange = [0, -8], rotRange = [0, Math.PI * 2], offset = 0 }) {
  ...
  const args = shape === 'torus' ? [1, 0.35, 32, 64]
    : shape === 'box' ? [1.4, 1.4, 1.4]
    : shape === 'sphere' ? [1, 48, 48]
    : [1, 1];
...
function SceneContent({ scroll }) {
  ...
  return (
    <>
      ...
      <directionalLight position={[-5, -3, 2]} intensity={1.1} color="#60a5fa" />
      <pointLight position={[0, 0, 5]} intensity={1.4} color="#34d399" />
      <pointLight position={[4, -4, 3]} intensity={1.0} color="#f472b6" />
      <hemisphereLight intensity={0.4} color="#a78bfa" groundColor="#050507" />
