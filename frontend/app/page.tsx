          ...
          <Reveal y={20}>
            <div className="text-center lg:text-left">
              ...
              <p className="mx-auto mt-6 max-w-xl text-pretty text-lg leading-relaxed text-txt-secondary lg:mx-0">
                NexusLLM is a free gateway, manager and chat playground. Point
                any agent at one base URL and key to reach every free LLM — with
                Auto routing and Fusion built in.
              </p>
              <div className="mt-9 flex flex-col items-center gap-3 sm:flex-row lg:justify-start">
                <Link
                  href="/signup"
                  className="group inline-flex h-12 items-center gap-2 rounded-full bg-txt-primary px-7 text-sm font-semibold text-bg-primary transition-transform hover:scale-[1.04] active:scale-[0.97]"
                >
                  Start free
                  <svg
          ...
          <Reveal>
            <div>
              ...
              <p className="mt-4 max-w-md text-txt-secondary">
                NexusLLM speaks the OpenAI API. Set the base URL and your key
                in any SDK or agent — the model id{" "}
                <code className="rounded bg-bg-tertiary/60 px-1.5 py-0.5 font-mono text-sm">
                  auto
                </code>{" "}
                just works.
              </p>
              <ul className="mt-8 space-y-4">
                {[
                  [
                    "Add free provider keys",
                    "Groq, Google, NVIDIA, Mistral and more — once, in the Keys page.",
                  ],
                  [
                    "Copy your unified key",
                    "One key fronts them all. Rotate or revoke anytime.",
                  ],
                  [
                    "Connect any agent",
                    "OpenCode, Claude Code, Cursor, Cline — paste base URL + key.",
                  ],
                ].map(([h, d], i) => (
      ...
      <section className="relative mx-auto max-w-4xl px-5 py-32 text-center lg:px-8">
        <Reveal>
          ...
          <div className="mt-10 flex flex-col items-center justify-center gap-3 sm:flex-row">
            <Link
              href="/signup"
              className="inline-flex h-12 items-center rounded-full bg-txt-primary px-8 text-sm font-semibold text-bg-primary transition-transform hover:scale-[1.04] active:scale-[0.97]"
            >
              Create your account
            </Link>
            <a
      ...
      <footer className="relative border-t border-border bg-bg-secondary/40 backdrop-blur">
        <div className="mx-auto flex max-w-6xl flex-col items-center justify-between gap-6 px-5 py-10 sm:flex-row lg:px-8">
          ...
          <nav className="flex flex-wrap items-center justify-center gap-x-6 gap-y-2 text-sm text-txt-secondary">
            <Link href="/docs" className="hover:text-txt-primary">
              Docs
            </Link>
            <Link href="/models" className="hover:text-txt-primary">
              Models
            </Link>
            <Link href="/chat" className="hover:text-txt-primary">
              Chat
            </Link>
            <Link href="/playground" className="hover:text-txt-primary">
              Playground
            </Link>
            ...
            <a
              href="https://github.com/khushnawriya30-del/nexusllm"
...
function FeatureCard({ f }: { f: { title: string; body: string; icon: string } }) {
  return (
    <div className="group h-full rounded-3xl border border-border bg-bg-secondary/40 p-7 backdrop-blur transition-all duration-300 hover:-translate-y-1.5 hover:border-border-hover hover:shadow-lg">
